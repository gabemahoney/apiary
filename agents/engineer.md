...
- **External-reference tickets.** When the ticket's `source_references` is non-empty and points at an external URL (e.g., `[{"value":"https://github.com/.../issues/123","resolver":"github-issue"}]` or `[{"value":"...","resolver":"linear-issue"}]` or `[{"value":"...","resolver":"url"}]`), the ticket body may be intentionally thin (a 2-3 sentence summary); the authoritative spec content lives at the URL. Fetch the upstream content and treat what you read as the spec source for the implementation. A concrete resolver implementation may not yet be registered for the resolver name written into `source_references` — a direct URL fetch is the canonical fallback path until a real resolver lands. If the upstream URL cannot be reached (network policy, auth-gated source, etc.), surface the failure to the orchestrator rather than guessing — the dispatch prompt's embedded body alone is not enough on this path.
